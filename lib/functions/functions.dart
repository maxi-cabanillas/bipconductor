...
darktheme() async {
  if (isDarkTheme == true) {
    ...
    buttonColor = buttonColorDarkMood;  // Igual que Rider
    loaderColor = Colors.white;
    hintColor = Colors.white.withOpacity(0.3);

    // Modo oscuro (Uber style)
    mapStyle = await rootBundle.loadString('assets/dark.json');

  } else {
    page = Colors.white;
    textColor = Colors.black;
    buttonColor = buttonColorLightMood;  // Igual que Rider
    loaderColor = theme;
    hintColor = const Color(0xff12121D).withOpacity(0.3);

    // Modo claro (mapa Google normal a color)
    mapStyle = await rootBundle.loadString('assets/map_default.json');
  }
...
getPolylines(animate) async {
  ...
  if (driverReq.isEmpty ||
      ...
      for (var i = 1; i < addressList.length; i++) {
        ...
        try {
          ...
          if (value.statusCode == 200) {
            ...
            ['encodedPolyline'];
            final int _beforeLen = polyList.length;
            decodeEncodedPolyline(steps.toString());
            // Evita punto duplicado en el empalme entre tramos
            if (_beforeLen > 0 && polyList.length > _beforeLen) {
              final a = polyList[_beforeLen - 1];
              final b = polyList[_beforeLen];
              if (a.latitude == b.latitude && a.longitude == b.longitude) {
                polyList.removeAt(_beforeLen);
              }
            }
          } else {}
        } catch (e) {
    ...
    for (var i = 0; i < poly.length; i++) {
      final int _beforeLen = polyList.length;
      decodeEncodedPolyline(poly[i].toString());
      if (_beforeLen > 0 && polyList.length > _beforeLen) {
        final a = polyList[_beforeLen - 1];
        final b = polyList[_beforeLen];
        if (a.latitude == b.latitude && a.longitude == b.longitude) {
...
bool _routeRebuildInFlight = false;

/// Recalcula y redibuja la polilínea desde la ubicación actual del conductor (center)
/// hacia el próximo punto objetivo (pickup si todavía no llegó, o el siguiente stop/drop pendiente).
/// Se puede llamar en cada update de GPS para que la ruta se actualice siempre.
Future<void> rebuildRoutePolylinesFromCurrent() async {
  if (_routeRebuildInFlight) return;
  _routeRebuildInFlight = true;

  try {
    if (driverReq.isEmpty || driverReq['accepted_at'] == null) return;
    if (center == null) return;
    if (addressList.isEmpty) return;

    final LatLng start =
    (center is LatLng) ? center as LatLng : LatLng(center.latitude, center.longitude);
    ...
    // Armar lista de destinos (pickup o próximos stops/drop sin completar)
    List<AddressList> targets = [];

    final arrived = driverReq['is_driver_arrived'];
    if (arrived == null || arrived == 0) {
      try {
        targets.add(addressList.firstWhere((e) => e.type == 'pickup'));
      } catch (_) {
        targets.add(addressList.first);
      }
    } else {
      targets = addressList
          .where((e) => e.type != 'pickup' && e.completedat == null)
          .toList();

      if (targets.isEmpty) {
        try {
          targets = [addressList.firstWhere((e) => e.type == 'drop')];
        } catch (_) {
          targets = [addressList.last];
        }
      }
    }
    ...
    for (final dest in targets) {
      ...
      if (resp.statusCode == 200) {
        ...
        if (decoded['routes'] != null && decoded['routes'].isNotEmpty) {
          final routeEncoded = decoded['routes']?[0]?['polyline']?['encodedPolyline'];
          final int _beforeLen = polyList.length;
          if (routeEncoded != null && routeEncoded.toString().isNotEmpty) {
            decodeEncodedPolyline(routeEncoded.toString());
          } else {
            // Fallback: si no viene polyline completa, usar steps
            final steps = decoded['routes'][0]['legs'][0]['steps'] as List<dynamic>;
            for (var j = 0; j < steps.length; j++) {
              final encoded = steps[j]['polyline']?['encodedPolyline'];
              if (encoded != null && encoded.toString().isNotEmpty) {
                decodeEncodedPolyline(encoded.toString());
              }
            }
          }
          // Evita punto duplicado al empalmar segmentos
          if (_beforeLen > 0 && polyList.length > _beforeLen) {
            final a = polyList[_beforeLen - 1];
            final b = polyList[_beforeLen];
            if (a.latitude == b.latitude && a.longitude == b.longitude) {
...
List<PointLatLng> decodeEncodedPolyline(String encoded) {
  ...
  // NO dibujar polyline acá (evita duplicados). Solo decodificamos puntos.
  return poly;
}
...
void updateDriverLocationSettings({
  ...
  final useBoost =
      _locationBoostUntil != null && now.isBefore(_locationBoostUntil!);
  final key = '$mode|boost=$useBoost';
  if (key == _locationSettingsKey) return;

  if (platform == TargetPlatform.android) {
    final accuracySetting = useBoost
...
// -----------------------------------------------------------------------------
// Hotfix: some code paths call relaunchApp(); older versions of this project
// referenced a plugin method that may not exist anymore.
// Keeping this as a safe no-op (or soft restart) prevents build failures.
// -----------------------------------------------------------------------------
void relaunchApp() {
  // If you want a real "rebirth", use flutter_phoenix and call Phoenix.rebirth(context).
  // Here we keep it safe to avoid crashes/build issues.
  try {
    // Best-effort: close the app (Android). Comment out if you don't want this behavior.
    // SystemNavigator.pop();
  } catch (_) {}
}
