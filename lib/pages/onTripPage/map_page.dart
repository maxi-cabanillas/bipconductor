...
class _MapsState extends State<Maps>
  ...
  // =============================================================
  // ZONAS PELIGROSAS / PRECAUCIÓN (Google My Maps)
  // Fuente: https://goo.gl/maps/xXJzQs9H6XNwn8bm8
  // Se carga como KML público desde My Maps y se renderiza como Polygons/Markers.
  // =============================================================
  static const String _dangerZonesKmlUrl =
      'https://www.google.com/maps/d/kml?forcekml=1&mid=1cHf6_AvF2KQ7n64qGrruVmXY7T9-nlQ';

  bool _showDangerZones = true;
  bool _dangerZonesLoading = false;
  String? _dangerZonesError;
  DateTime? _dangerZonesLoadedAt;

  // Google Maps overlays
  Set<Polygon> _dangerPolygons = <Polygon>{};
  Set<Circle> _dangerCircles = <Circle>{};
  Set<Marker> _dangerMarkers = <Marker>{};

  // FlutterMap overlays (OSM)
  List<fm.Polygon> _dangerFmPolygons = <fm.Polygon>[];
  List<fm.CircleMarker> _dangerFmCircles = <fm.CircleMarker>[];
  List<fm.Marker> _dangerFmMarkers = <fm.Marker>[];


  List driverData = [];
  double _isbottom = -1000;

  // Dock flotante +/− (Inicio/Historia/Ganancias/Viajes)
  bool _navDockExpanded = false;


  // =============================================================
  // NAVBAR SAFE INSET REAL (Android barra transparente / edge-to-edge)
  // Fuerza a respetar la barra de navegación aunque MediaQuery.padding sea 0.
  // =============================================================
  double _systemBottomInset() {
    // Queremos el inset REAL de la barra de navegación (Android), sin depender de MediaQuery "pisado".
    // Regla:
    // - Si el sistema reporta un viewPadding.bottom > 0, usarlo.
    // - Si reporta 0 (algunos Samsung/edge-to-edge), usar fallback SOLO si NO hay gesto (3 botones).
    final mq = MediaQuery.maybeOf(context);
    final mqBottom = mq?.viewPadding.bottom ?? 0.0;

    double viewBottom = 0.0;
    double gestureBottom = 0.0;
    ...
    try {
      ...
      if (views.isNotEmpty) {
        ...
        if (dpr > 0) {
          viewBottom = v.viewPadding.bottom / dpr;
          gestureBottom = v.systemGestureInsets.bottom / dpr;
        }
      }
    } catch (_) {}

    // Legacy fallback (por si platformDispatcher falla)
    double legacyBottom = 0.0;
    try {
      final w = WidgetsBinding.instance.window;
      final dpr = w.devicePixelRatio;
      if (dpr > 0) legacyBottom = w.viewPadding.bottom / dpr;
    } catch (_) {}

    final bottom = [mqBottom, viewBottom, legacyBottom].reduce((a, b) => a > b ? a : b);

    if (bottom > 0.5) {
      return bottom;
    }

    // Si no hay insets reportados:
    // - En Android con navegación por gestos, NO empujar (evita que quede muy arriba).
    // - En Android con 3 botones (gestureBottom ~ 0), usar un mínimo seguro.
    final tp = Theme.of(context).platform;
    if (tp == TargetPlatform.android) {
      final bool likelyThreeButtons = gestureBottom < 0.5;
      return likelyThreeButtons ? 56.0 : 0.0;
    }
    return 0.0;
  }
  ...
  DateTime? _followPausedUntil; // pauses auto-follow after user gestures
  bool _isProgrammaticCameraMove = false; // ignore programmatic camera move start
  bool _ignoreInitialCameraMoveStarted = true; // ignore first camera move started triggered by map init
  ...
  // ============================
  // BIP Voz (IA) - Avisos TTS
  // ============================
  late final _BippTtsService _bippTts;
  late final _BippTripVoiceAnnouncer _bippVoice;
  Timer? _bippVoiceHeartbeat;
  String? _bippVoiceTripKey;
  bool _bippVoiceCancelLatched = false;

  // IA Voz: throttle y no-bloqueo (evita lag al volver de Waze/Maps)
  int _bippVoiceLastLocMs = 0;
  bool _bippVoiceLocInFlight = false;
  LatLng? _bippVoiceLastPos;

  // ============================
  // BIP Performance (resume/waze)
  // ============================
  // Cuando volvés desde Waze/Google Maps, suspendemos operaciones pesadas 2s
  // (snap a polyline, animaciones de cámara, recalculo de ruta) para evitar “tildado”.
  bool _bippHeavyOpsSuspended = false;

  // 🔧 PERF: permite cortar operaciones pesadas cuando volvés de Waze/Maps
  bool get _heavyOpsAllowed => !_bippHeavyOpsSuspended && !isBackground;
  DateTime? _bippLastResumedAt;
  Timer? _bippResumeWarmupTimer;
  ...
  bool _bippTopPanelExpanded = true;
  ...
  void initState() {
    ...
    // ============================
    // BIP Voz (IA) - init TTS + watchdog
    // ============================
    _bippTts = _BippTtsService();
    _bippVoice = _BippTripVoiceAnnouncer(_bippTts);
    // No bloquea: inicializa en background
    _bippTts.init();

    // Watchdog para anunciar "Nuevo viaje" aunque el chofer esté quieto
    _bippVoiceHeartbeat?.cancel();
    _bippVoiceHeartbeat = Timer.periodic(const Duration(milliseconds: 900), (_) {
      _bippVoiceHeartbeatTick();
    });


    // Zonas peligrosas / precaución (My Maps)
    _loadDangerZonesKml();
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) async {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      // 🔧 PERF: al volver desde Waze/Google Maps, evitamos “tildado” por 2s
      _bippHeavyOpsSuspended = true;
      _bippLastResumedAt = DateTime.now();
      _bippResumeWarmupTimer?.cancel();
      _bippResumeWarmupTimer = Timer(const Duration(seconds: 2), () {
        if (!mounted) return;
        _bippHeavyOpsSuspended = false;

        // Re-sincroniza una vez la ruta si hay viaje activo (sin spamear)
        if (driverReq.isNotEmpty && driverReq['accepted_at'] != null) {
          handleRouteDeviationAndSnap(offRouteMeters: 50);
        }
      });

      // Reinicia heartbeat de voz (IA) al volver a foreground
      _bippVoiceHeartbeat?.cancel();
      _bippVoiceHeartbeat = Timer.periodic(const Duration(milliseconds: 900), (_) {
        _bippVoiceHeartbeatTick();
      });

      // Refresh rápido de UI (sin animar cámara) para que el botón “Finalizar” responda YA
      Future.microtask(() async {
        try {
          ...
          _updateDriverMarker(ll, heading);
          valueNotifierHome.incrementNotifier();
        } catch (_) {}
      });
      ...
      // Keep screen ON always (driver app should never sleep while in foreground)
      _setKeepScreenOn(true);
      // DashBubble.instance.stopBubble();
      QuickNav.I.stopService();
      isBackground = true;
      ...
      // 🔧 PERF: al ir a background (Waze/Maps), parar timers pesados
      _bippHeavyOpsSuspended = true;
      _bippResumeWarmupTimer?.cancel();
      _bippVoiceHeartbeat?.cancel();
  ...
  Future<void> _startLiveDriverTracking() async {
    ...
    try {
      ...
      final bool _navActive = driverReq.isNotEmpty;
      final settings = Platform.isAndroid
          ? geolocator.AndroidSettings(
        accuracy: _navActive
            ? geolocator.LocationAccuracy.bestForNavigation
            : geolocator.LocationAccuracy.high,
        // Más rápido en navegación para que no se atrase la flecha.
        distanceFilter: _navActive ? 1 : 4,
        intervalDuration: _navActive
      ...
      _livePosSub =
          geolocator.Geolocator.getPositionStream(locationSettings: settings)
            ...
            if (_lastLivePosHandledAt != null &&
                now.difference(_lastLivePosHandledAt!).inMilliseconds < (_navActive ? 350 : 700)) {
              return;
            }
            _lastLivePosHandledAt = now;

            final double snapMaxDistanceMeters = _navActive ? 12.0 : 18.0;
            ...
            final bool _heavyOpsAllowed = !_bippHeavyOpsSuspended && !isBackground;
            ...
            if (_heavyOpsAllowed) {
              ...
              final shouldSnap = snapCandidate != null &&
                  // No snaps si el GPS viene con mucha inexactitud (evita atraso y saltos).
                  (pos.accuracy.isNaN || pos.accuracy <= 35) &&
                  // Si vas parado o muy lento, no forces snap (tiende a quedarse atrás).
                  (pos.speed.isNaN || pos.speed >= 1.0) &&
            ...
            // 🔊 BIP Voz (IA): avisos por distancia/estado
            _bippVoiceOnLocation(displayLatLng, accuracyMeters: accuracy);
// 🔁 Recalcular y redibujar la ruta en cada update de GPS
            ...
            if (_heavyOpsAllowed && driverReq.isNotEmpty && driverReq['accepted_at'] != null) {
              // Navigation mode: keep centered; auto-resume after pause
              if (_followPausedUntil == null || DateTime.now().isAfter(_followPausedUntil!)) {
                _followDriver = true;
                _followBearing = true;
              }
            ...
            // Auto-resume follow after user gestures (also in idle mode).
            if (_followPausedUntil != null && DateTime.now().isAfter(_followPausedUntil!)) {
              _followDriver = true;
              _followBearing = true;
            ...
            if (shouldUpdateUi) {
              ...
              double _appliedBearing = newHeading;

              if (_heavyOpsAllowed && _followDriver) {
                _appliedBearing = _animateToDriver(displayLatLng, newHeading);
              }

              _updateDriverMarker(displayLatLng, _appliedBearing);
              valueNotifierHome.incrementNotifier();
            }
          });
    } catch (_) {}
  ...
  bool _shouldUpdateDriverUi(LatLng latLng, double newHeading) {
    ...
    final bool navMode = _followDriver || (driverReq.isNotEmpty && driverReq['accepted_at'] != null);

    // En navegación queremos que la flecha sea más "real" (actualiza con menos metros).
    // En modo idle mantenemos un umbral mayor para no gastar batería de más.
    final double moveThreshold = navMode ? 1.8 : 3.5;
    final double headingThreshold = navMode ? 2.0 : 3.0;

    return moved >= moveThreshold || headingDelta >= headingThreshold;
  }
  ...
  // ============================
  // Single route styling (Google Maps) - ONE polyline only
  // ============================
  void _applyWazePolylineStyle() {
    try {
      ...
      LatLng? asLatLng(dynamic p) {
        try {
          if (p is LatLng) return p;
          if (p is Map) {
            final lat = double.tryParse(p['latitude']?.toString() ?? p['lat']?.toString() ?? '');
            final lng = double.tryParse(p['longitude']?.toString() ?? p['lng']?.toString() ?? '');
      ...
      final LatLng? driverLL = _lastUiDriverLatLng ?? _prevDriverLatLng;
      if (driverLL != null) {
        int bestIdx = 0;
        double bestD = double.infinity;

        final int step = (pts.length / 220).ceil().clamp(1, 50);
        ...
        if (bestD <= 400.0 && bestIdx > 0 && bestIdx < pts.length - 1) {
          pts.removeRange(0, bestIdx);
        }
      }


      // ✅ ROUTE STAGE TRIM (evita doble línea sin matar el preview)
      // - Pedido nuevo (preview, sin aceptar): mantener driver→pickup→drop (no recortar)
      // - Aceptado, aún no iniciado: recortar en pickup (solo driver→pickup)
      // - Viaje iniciado: mantener hasta drop
      try {
        final bool hasAccepted =
        (driverReq.isNotEmpty && driverReq['accepted_at'] != null);

        final bool tripStarted = hasAccepted
            ? ((driverReq['is_trip_start'] == true) ||
            (driverReq['is_trip_start'] == 1) ||
            (driverReq['is_trip_start'] == '1'))
            : false;

        // Solo recortamos cuando el pedido está ACEPTADO y todavía estamos yendo al pickup.
        if (hasAccepted && !tripStarted) {
          dynamic rawLat;
          dynamic rawLng;

          // Pickup preferido desde driverReq
          rawLat = driverReq['pick_lat'] ??
              driverReq['pickup_lat'] ??
              driverReq['pickup_latitude'] ??
              driverReq['pick_latitude'];
          rawLng = driverReq['pick_lng'] ??
              driverReq['pickup_lng'] ??
              driverReq['pickup_longitude'] ??
              driverReq['pick_longitude'];

          // Fallback: si por alguna razón no vino en driverReq, probamos choosenRide[0]
          if ((rawLat == null || rawLng == null) && choosenRide.isNotEmpty) {
            final dynamic r0 = choosenRide[0];
            if (r0 is Map) {
              rawLat = rawLat ??
                  r0['pick_lat'] ??
                  r0['pickup_lat'] ??
          ...
          final double? tLat = _p(rawLat);
          final double? tLng = _p(rawLng);

          if (tLat != null && tLng != null && pts.length > 2) {
            final target = LatLng(tLat, tLng);

            int bestIdx2 = -1;
            double bestD2 = double.infinity;

            final int step2 = (pts.length / 220).ceil().clamp(1, 50);
            for (int i = 0; i < pts.length; i += step2) {
              final p = pts[i];
              final d = geolocator.Geolocator.distanceBetween(
                target.latitude,
                target.longitude,
            ...
            // Si ubicamos el pickup sobre la ruta, cortamos todo lo que viene después.
            if (bestIdx2 >= 0 && bestD2 <= 120.0 && bestIdx2 < pts.length - 1) {
              pts.removeRange(bestIdx2 + 1, pts.length);
            }
          }
        }
      } catch (_) {}
  ...
  /// Devuelve un identificador estable del pedido/viaje actual para comparaciones.
  /// - driverReq suele ser Map
  /// - choosenRide suele ser List donde choosenRide[0] es Map
  String _bippCurrentRideKey() {
    try {
      if (driverReq.isNotEmpty) {
        final dynamic v = driverReq['id'] ?? driverReq['request_id'] ?? driverReq['trip_id'];
        if (v != null) return v.toString();
      }
      if (choosenRide.isNotEmpty) {
        final dynamic r0 = choosenRide[0];
        if (r0 is Map) {
          final dynamic v = r0['id'] ?? r0['request_id'] ?? r0['trip_id'];
          if (v != null) return v.toString();
        }
      }
    } catch (_) {}
  ...
  Future<void> handleRouteDeviationAndSnap({double offRouteMeters = 50}) async {
    ...
    // Si no hay ruta (o se borró la polyline), forzamos rebuild.
    final len = polyList.length;
    if (len < 2 || polyline.isEmpty) {
      _routeRebuildInProgress = true;
      _lastRouteRebuildAt = DateTime.now();
      () async {
        try {
...
// Sample the polyline to keep this cheap.
  ...
  double _animateToDriver(LatLng latLng, double newHeading) {
    if (_controller == null) return _cameraBearing;

    final now = DateTime.now();
    if (_lastCameraMove != null &&
        now.difference(_lastCameraMove!).inMilliseconds < 250) {
      return _cameraBearing; // throttle camera work
    }
    _lastCameraMove = now;
  ...
  void _onCameraMoveStarted() {
    ...
    // GoogleMap may fire this once on init; don't disable follow for that.
    if (_ignoreInitialCameraMoveStarted && _prevDriverLatLng == null) {
      _ignoreInitialCameraMoveStarted = false;
      return;
    }
    _ignoreInitialCameraMoveStarted = false;
    ...
    _followPausedUntil = DateTime.now().add(const Duration(seconds: 6));
  }

  void _onCameraIdle() {
    // Auto-resume follow even in idle mode (so you don't need to press "center" again).
    if (_followPausedUntil == null) return;
    if (DateTime.now().isAfter(_followPausedUntil!)) {
      _followDriver = true;
      _followBearing = true;
      _followPausedUntil = null;
    }
  ...
  // ===================== ZONAS PELIGROSAS (KML -> overlays) =====================

  Future<void> _loadDangerZonesKml({bool force = false}) async {
    if (!_showDangerZones) return;
    if (_dangerZonesLoading) return;

    // cache simple (1 hora)
    if (!force &&
        _dangerZonesLoadedAt != null &&
        DateTime.now().difference(_dangerZonesLoadedAt!).inMinutes < 60) {
      return;
    }

    setState(() {
      _dangerZonesLoading = true;
    ...
    try {
      ...
      final res = await http
          .get(Uri.parse(_dangerZonesKmlUrl))
          .timeout(const Duration(seconds: 12));

      if (res.statusCode != 200 || res.body.isEmpty) {
        throw Exception('No pude descargar KML (HTTP ${res.statusCode}).');
      }
      body = res.body;

      // A veces MyMaps devuelve un NetworkLink. Seguimos el href 1 vez.
      final hrefMatch = RegExp(r'<href>\s*([^<]+)\s*</href>')
          .firstMatch(body);
      if (hrefMatch != null && body.contains('<NetworkLink')) {
      ...
      final parsed = _parseDangerZonesFromKml(body);

      setState(() {
        _dangerPolygons = parsed.googlePolygons;
        _dangerCircles = <Circle>{};
        _dangerMarkers = <Marker>{};
        _dangerFmPolygons = parsed.fmPolygons;
        _dangerFmCircles = <fm.CircleMarker>[];
        _dangerFmMarkers = <fm.Marker>[];
        _dangerZonesLoadedAt = DateTime.now();
        _dangerZonesLoading = false;
      });
    } catch (e) {
      setState(() {
        _dangerZonesError = e.toString();
        _dangerZonesLoading = false;
      });
    }
  }

  void _onDangerZoneTap(String title) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Zona: $title'),
        duration: const Duration(seconds: 2),
      ),
    );
  }

  void _showRideTakenMessage([String msg = 'Este viaje ya fue tomado por otro conductor.']) {
    if (!mounted) return;
...
// IA Voz: también avisar por voz (así el chofer no se queda sin feedback)
    final tk = _bippComputeTripKey();
    if (tk != null) {
      if (_bippVoiceTripKey != tk) {
        _bippVoiceTripKey = tk;
        _bippVoice.resetForTrip(tk);
      }
      Future.microtask(() => _bippVoice.onTakenByOther());
    } else {
      // fallback genérico
      Future.microtask(() => _bippTts.speak(msg, key: "taken_generic", cooldownMs: 2500, force: true));
    }
  }


  _DangerZonesParsed _parseDangerZonesFromKml(String kml) {
    // 1) Styles (id -> style)
    final Map<String, _KmlStyle> styles = {};
    final Map<String, String> styleMaps = {}; // StyleMapId -> normalStyleId

    for (final sm in RegExp(r'<StyleMap[^>]*id="([^"]+)"[^>]*>([\s\S]*?)</StyleMap>')
        .allMatches(kml)) {
      final id = sm.group(1) ?? '';
      final body = sm.group(2) ?? '';
      final normal = RegExp(r'<Pair>\s*<key>normal</key>[\s\S]*?<styleUrl>#([^<]+)</styleUrl>')
          .firstMatch(body)
          ?.group(1);
      if (id.isNotEmpty && normal != null && normal.isNotEmpty) {
        styleMaps[id] = normal;
      }
    }

    for (final s in RegExp(r'<Style[^>]*id="([^"]+)"[^>]*>([\s\S]*?)</Style>')
        .allMatches(kml)) {
      final id = s.group(1) ?? '';
      final body = s.group(2) ?? '';
      ...
      final lineColor = RegExp(r'<LineStyle>[\s\S]*?<color>\s*([^<]+)\s*</color>')
          .firstMatch(body)
          ?.group(1);
      final lineWidth = RegExp(r'<LineStyle>[\s\S]*?<width>\s*([^<]+)\s*</width>')
          .firstMatch(body)
          ?.group(1);

      final polyColor = RegExp(r'<PolyStyle>[\s\S]*?<color>\s*([^<]+)\s*</color>')
          .firstMatch(body)
          ?.group(1);

      if (lineColor != null) stroke = _kmlColorToDart(lineColor);
      if (polyColor != null) fill = _kmlColorToDart(polyColor);
      if (lineWidth != null) {
        final w = int.tryParse(lineWidth.trim());
        if (w != null && w > 0) strokeWidth = w;
      }
    ...
    // 2) Placemarks
    final Set<Polygon> gPolys = {};
    final Set<Circle> gCircles = {};
    final Set<Marker> gMarkers = {};

    final List<fm.Polygon> fmPolys = [];
    final List<fm.CircleMarker> fmCircles = [];
    final List<fm.Marker> fmMarkers = [];

    int polyIdx = 0;
    int ptIdx = 0;

    for (final pm in RegExp(r'<Placemark[^>]*>([\s\S]*?)</Placemark>')
        .allMatches(kml)) {
      final body = pm.group(1) ?? '';
      final name = _kmlText(body, 'name') ?? 'Zona';
      final desc = _kmlText(body, 'description');

      String? styleId;
      final styleUrl = RegExp(r'<styleUrl>\s*#([^<]+)\s*</styleUrl>')
          .firstMatch(body)
          ?.group(1)
          ?.trim();
      if (styleUrl != null && styleUrl.isNotEmpty) {
        styleId = styleMaps[styleUrl] ?? styleUrl;
      }

      final st = (styleId != null) ? styles[styleId] : null;

      // Heurística por nombre si no hay estilo
      final bool isPrecaucion = RegExp(r'precauc', caseSensitive: false).hasMatch(name);
      final Color fallbackStroke = isPrecaucion ? Colors.orange : Colors.red;
      final Color stroke = (st?.stroke ?? fallbackStroke);
      final Color fill = (st?.fill ?? fallbackStroke.withOpacity(0.18));
      final int strokeW = st?.strokeWidth ?? 2;

      // Polygon (outer boundary)
      final polyMatch = RegExp(r'<Polygon[\s\S]*?<outerBoundaryIs>[\s\S]*?<coordinates>([\s\S]*?)</coordinates>')
          .firstMatch(body);
      if (polyMatch != null) {
        final coordsRaw = polyMatch.group(1) ?? '';
        final pts = _parseKmlCoordinates(coordsRaw);
        if (pts.length >= 3) {
          final pid = PolygonId('danger_poly_${polyIdx++}');
          gPolys.add(
            Polygon(
              polygonId: pid,
              points: pts,
          ...
          fmPolys.add(
            fm.Polygon(
              points: pts.map((e) => fmlt.LatLng(e.latitude, e.longitude)).toList(),
              color: fill,
              borderColor: stroke,
              borderStrokeWidth: strokeW.toDouble(),
            ),
          );
      ...
      // Point (si hay puntos sueltos, los marcamos + circulito)
      final pointMatch = RegExp(r'<Point[\s\S]*?<coordinates>\s*([^<]+)\s*</coordinates>')
          .firstMatch(body);
      if (false && pointMatch != null) {
        final coords = pointMatch.group(1) ?? '';
        final pts = _parseKmlCoordinates(coords);
        if (pts.isNotEmpty) {
          final p = pts.first;
          final mid = MarkerId('danger_point_${ptIdx++}');
          gMarkers.add(
            Marker(
              markerId: mid,
              position: p,
              infoWindow: InfoWindow(title: name, snippet: (desc ?? '').replaceAll(RegExp(r'<[^>]+>'), '').trim()),
              onTap: () => _onDangerZoneTap(name),
            ),
          );
          ...
          gCircles.add(
            Circle(
              circleId: CircleId('danger_circle_${ptIdx}'),
              center: p,
              radius: 140, // metros (área de referencia)
              fillColor: fill,
              strokeColor: stroke,
              strokeWidth: strokeW,
              consumeTapEvents: true,
          ...
          fmCircles.add(
            fm.CircleMarker(
              point: fmlt.LatLng(p.latitude, p.longitude),
              radius: 18, // visual (px aprox)
              color: fill,
              borderColor: stroke,
              borderStrokeWidth: strokeW.toDouble(),
            ),
    ...
    return _DangerZonesParsed(
      googlePolygons: gPolys,
      googleCircles: gCircles,
      googleMarkers: gMarkers,
      fmPolygons: fmPolys,
      fmCircles: fmCircles,
      fmMarkers: fmMarkers,
    );
  ...
  String? _kmlText(String body, String tag) {
    final m = RegExp('<$tag>([\s\S]*?)</$tag>').firstMatch(body);
    if (m == null) return null;
    var t = m.group(1) ?? '';
    // Limpieza básica
    t = t.replaceAll('<![CDATA[', '').replaceAll(']]>', '');
    t = t.replaceAll(RegExp(r'<[^>]+>'), '');
    return t.trim();
  }

  List<LatLng> _parseKmlCoordinates(String raw) {
    final out = <LatLng>[];
    final s = raw
        .replaceAll('\n', ' ')
        .replaceAll('\r', ' ')
    ...
    for (final p in parts) {
      ...
      final lng = double.tryParse(nums[0].trim());
  ...
  Color _kmlColorToDart(String kmlColor) {
    ...
    if (hex.length == 6) {
      // bb ggrr? (si viene sin alpha, asumimos ff)
      final bb = hex.substring(0, 2);
      final gg = hex.substring(2, 4);
      final rr = hex.substring(4, 6);
      return Color(int.parse('0xFF$rr$gg$bb'));
    }
    // fallback rojo suave
  ...
  // ============================================================================
  // Request UI helpers (KM a destino + seguridad Origen/Destino por zonas peligrosas)
  // ============================================================================

  double _pickupToDropKm() {
    try {
      if (driverReq.isEmpty) return 0.0;
      final double pl = double.tryParse(driverReq['pick_lat']?.toString() ?? '') ?? 0.0;
      final double pg = double.tryParse(driverReq['pick_lng']?.toString() ?? '') ?? 0.0;
      final double dl = double.tryParse(driverReq['drop_lat']?.toString() ?? '') ?? 0.0;
      final double dg = double.tryParse(driverReq['drop_lng']?.toString() ?? '') ?? 0.0;
      if ((pl == 0.0 && pg == 0.0) || (dl == 0.0 && dg == 0.0)) return 0.0;

      final double meters = geolocator.Geolocator.distanceBetween(pl, pg, dl, dg);
      if (meters.isNaN || !meters.isFinite) return 0.0;
  ...
  bool _pointInPolygon(LatLng p, List<LatLng> poly) {
    ...
    for (int i = 0, j = poly.length - 1; i < poly.length; j = i++) {
      final double xi = poly[i].latitude;
      final double yi = poly[i].longitude;
      final double xj = poly[j].latitude;
      final double yj = poly[j].longitude;

      final bool intersect = ((yi > p.longitude) != (yj > p.longitude)) &&
          (p.latitude < (xj - xi) * (p.longitude - yi) / ((yj - yi) == 0 ? 1e-12 : (yj - yi)) + xi);
  ...
  bool _isDangerousLatLng(double lat, double lng) {
    try {
      if (_dangerPolygons.isEmpty) return false; // si todavía no cargó KML => seguro
      final p = LatLng(lat, lng);
      for (final poly in _dangerPolygons) {
  ...
  bool _isDangerousPickup() {
    final double pl = double.tryParse(driverReq['pick_lat']?.toString() ?? '') ?? 0.0;
    final double pg = double.tryParse(driverReq['pick_lng']?.toString() ?? '') ?? 0.0;
    if (pl == 0.0 && pg == 0.0) return false;
    return _isDangerousLatLng(pl, pg);
  }

  bool _isDangerousDrop() {
    final double dl = double.tryParse(driverReq['drop_lat']?.toString() ?? '') ?? 0.0;
    final double dg = double.tryParse(driverReq['drop_lng']?.toString() ?? '') ?? 0.0;
    if (dl == 0.0 && dg == 0.0) return false;
    return _isDangerousLatLng(dl, dg);
  }
  @override
  void dispose() {
  ...
  void _bippVoiceHeartbeatTick() {
    // Detecta nuevo viaje aunque el chofer esté quieto
    final tripKey = _bippComputeTripKey();
    if (tripKey == null) {
      _bippVoiceTripKey = null;
      _bippVoiceCancelLatched = false;
      return;
    }

    // Si cambió el viaje, resetea flags y anuncia si corresponde
    if (_bippVoiceTripKey != tripKey) {
      _bippVoiceTripKey = tripKey;
      _bippVoice.resetForTrip(tripKey);

      // Nuevo pedido: preferimos anunciar cuando aún no está aceptado
      if (!_bippIsAccepted()) {
        // no await: no bloquear UI
        Future.microtask(() => _bippVoice.onNewTrip(tripKey));
      }
    }

    // Cancelación latch (por si el cancel llega sin llamar reqCancel por algún motivo)
    if (_reqCancelled && !_bippVoiceCancelLatched) {
      _bippVoiceCancelLatched = true;
      Future.microtask(() => _bippVoice.onCancelled());
    } else if (!_reqCancelled) {
      _bippVoiceCancelLatched = false;
    }
  }

  // IA Voz: no bloquear el stream de GPS (fire-and-forget + throttle).
// Esto evita que al volver de Waze/Google Maps la app quede pesada y “tildada”.
  void _bippVoiceOnLocation(LatLng pos, {double? accuracyMeters}) {
    if (!mounted) return;

    // Si la app está en background (ej: estás en Waze/Maps), no hacemos nada acá.
    // (La voz de esta app no se escucha y solo sumaría carga al volver.)
    if (isBackground) return;

    final nowMs = DateTime.now().millisecondsSinceEpoch;

    // Throttle por tiempo
    if (nowMs - _bippVoiceLastLocMs < 900) return;

    // Evita overlap: si ya hay un tick de voz corriendo, no dispares otro
    if (_bippVoiceLocInFlight) return;

    // Filtra GPS ruidoso si está disponible
    if (accuracyMeters != null && accuracyMeters > 60.0) return;

    // Throttle por movimiento mínimo (evita jitter)
    if (_bippVoiceLastPos != null) {
      try {
        final d = geolocator.Geolocator.distanceBetween(
          _bippVoiceLastPos!.latitude,
          _bippVoiceLastPos!.longitude,
          pos.latitude,
          pos.longitude,
        );
        if (d < 4.0) return;
    ...
    _bippVoiceLastLocMs = nowMs;
    _bippVoiceLastPos = pos;
    _bippVoiceLocInFlight = true;

    // Fire-and-forget (no await)
    Future.microtask(() async {
      try {
        await _bippVoiceOnLocationCore(pos);
      } catch (_) {
        // ignore
      } finally {
        _bippVoiceLocInFlight = false;
      }
    });
  }

  Future<void> _bippVoiceOnLocationCore(LatLng pos) async {
    final tripKey = _bippComputeTripKey();
    ...
    if (_bippVoiceTripKey != tripKey) {
      ...
      // Si es un pedido nuevo (no aceptado), avisar de inmediato
      if (!_bippIsAccepted()) {
        await _bippVoice.onNewTrip(tripKey);
      }
    }

    // Si canceló, no seguir hablando cosas del viaje
    if (_reqCancelled) return;

    final accepted = _bippIsAccepted();
    final tripStarted = _bippIsTripStarted();

    final pick = _bippPickupLatLng();
    final drop = _bippDropLatLng();

    // 150m antes de pickup (requiere pickup válido)
    if (accepted && !tripStarted && pick != null) {
      await _bippVoice.onLocationUpdate(
        myLat: pos.latitude,
        myLng: pos.longitude,
        pickLat: pick.latitude,
        pickLng: pick.longitude,
        dropLat: pick.latitude, // no se usa en este stage
        dropLng: pick.longitude,
        accepted: accepted,
    ...
    // 150m/50m antes de destino (requiere drop válido)
    if (tripStarted && drop != null) {
      final p = pick ?? pos;
      await _bippVoice.onLocationUpdate(
        myLat: pos.latitude,
        myLng: pos.longitude,
  ...
  void _bippVoiceNotifyCancelled() {
    final tripKey = _bippComputeTripKey();
    if (tripKey != null && _bippVoiceTripKey != tripKey) {
      _bippVoiceTripKey = tripKey;
      _bippVoice.resetForTrip(tripKey);
    }
    Future.microtask(() => _bippVoice.onCancelled());
  ...
  bool _bippIsAccepted() {
    try {
      return driverReq.isNotEmpty && driverReq['accepted_at'] != null;
    } catch (_) {
      return false;
    }
  }

  bool _bippIsTripStarted() {
    if (!_bippIsAccepted()) return false;
    final v = driverReq['is_trip_start'];
    return (v == true) || (v == 1) || (v == '1');
  }

  String? _bippComputeTripKey() {
    // 1) IDs directos si existen
    try {
      if (driverReq.isNotEmpty) {
        final id = driverReq['id'] ?? driverReq['request_id'] ?? driverReq['trip_id'];
        if (id != null) return id.toString();
      }
    } catch (_) {}

    try {
      if (choosenRide.isNotEmpty && choosenRide[0] is Map) {
        final m = choosenRide[0] as Map;
        final id = m['request_id'] ?? m['id'] ?? m['trip_id'];
        if (id != null) return id.toString();
      }
    } catch (_) {}

    // 2) Fallback: firma por coordenadas + usuario (evita repeats)
    final pick = _bippPickupLatLng();
    final drop = _bippDropLatLng();
    dynamic userId;
    try {
      final ud = driverReq['userDetail'];
      if (ud is Map && ud['id'] != null) userId = ud['id'];
    } catch (_) {}

    if (pick != null) {
      final p1 = pick.latitude.toStringAsFixed(5);
      final p2 = pick.longitude.toStringAsFixed(5);
      final d1 = drop?.latitude.toStringAsFixed(5) ?? '';
      final d2 = drop?.longitude.toStringAsFixed(5) ?? '';
      return 'k_${p1}_${p2}_${d1}_${d2}_u${userId ?? ''}';
    }

    return null;
  }

  LatLng? _bippPickupLatLng() {
    // prioridad: driverReq
    final lat = _bippPickLatFromDriverReq();
    final lng = _bippPickLngFromDriverReq();
    if (lat != null && lng != null) return LatLng(lat, lng);

    // fallback: choosenRide
    try {
      if (choosenRide.isNotEmpty && choosenRide[0] is Map) {
        final m = choosenRide[0] as Map;
        final clat = _bippAsDouble(m['pick_lat']);
        final clng = _bippAsDouble(m['pick_lng']);
  ...
  LatLng? _bippDropLatLng() {
    final lat = _bippDropLatFromDriverReq();
    final lng = _bippDropLngFromDriverReq();
    if (lat != null && lng != null) return LatLng(lat, lng);
    return null;
  }

  double? _bippPickLatFromDriverReq() => _bippFirstDouble(driverReq, const [
    'pick_lat',
    'pickup_lat',
    'pickup_latitude',
    'pick_latitude',
  ...
  double? _bippPickLngFromDriverReq() => _bippFirstDouble(driverReq, const [
    'pick_lng',
    'pickup_lng',
    'pickup_longitude',
    'pick_longitude',
  ...
  double? _bippDropLatFromDriverReq() => _bippFirstDouble(driverReq, const [
    'drop_lat',
    'drop_latitude',
  ]);

  double? _bippDropLngFromDriverReq() => _bippFirstDouble(driverReq, const [
    'drop_lng',
    'drop_longitude',
  ]);

  double? _bippFirstDouble(dynamic mapLike, List<String> keys) {
    try {
      if (mapLike is Map) {
        for (final k in keys) {
          final v = mapLike[k];
          final d = _bippAsDouble(v);
          if (d != null) return d;
        }
      }
    } catch (_) {}
  ...
  double? _bippAsDouble(dynamic v) {
    if (v == null) return null;
    if (v is double) return v;
    if (v is int) return v.toDouble();
    if (v is num) return v.toDouble();
  ...
  getPoly(change) async {
    fmpoly.clear();
    // getPoly() es SOLO para flutter_map. En Google Maps no debe tocar polylines.
    if (mapType == 'google') {
      fmPolyGot = false;
      return;
    }
...
//getting permission and current location
  ...
  Widget build(BuildContext context) {
    ...
    final double _safeBottomInset = _systemBottomInset();
// Bottom sheet (on-ride) sizing:
    ...
    // Queremos que TODOS los estados del viaje (buscar cliente / llegué / iniciar / finalizar)
    // tengan la MISMA altura y queden por arriba de la mitad de la pantalla.
    final double _panelMaxOpenVisible = media.height * 0.72;
    ...
    final bool _hasActiveTripPanel =
    (driverReq.isNotEmpty && driverReq['accepted_at'] != null);
    if (_hasActiveTripPanel) {
      // Arrancar el panel más abierto (misma altura en todas las fases)
      // para ver bien calle + total a pagar.
                ...
                element.markerId == const MarkerId('1'))
                  ...
                  if (dist > 0 &&
                    ...
                    if (mapType == 'google') {
                      ...
                      // NO recortar ni borrar polyList/polyline acá: era destructivo y hacía que la ruta desaparezca.
                      // Solo re-aplicamos estilo para mantener UNA sola línea estable.
                      _applyWazePolylineStyle();
                    } else {
                      List polys = [];
                      dynamic nearestLat;
                                          ...
                                          (state == '1')
                                            ...
                                            children: [
                                                    ...
                                                    return GoogleMap(
                                                      ...
                                                      padding: EdgeInsets.only(
                                                        bottom: (driverReq['accepted_at'] != null)
                                                            ? (_panelVisible + _safeBottomInset + 8)
                                                            : (media.width * 1),
                                                        top: _bippTopPadding,
                                                      ),
                                                      onMapCreated:
                                                      ...
                                                      polylines:
                                                      polyline,
                                                      polygons: _showDangerZones ? _dangerPolygons : <Polygon>{},
                                                      ...
                                                      return SizedBox(
                                                        ...
                                                        child: fm
                                                          ...
                                                          children: [
                                                            ...
                                                            if (_showDangerZones && _dangerFmPolygons.isNotEmpty)
                                                              fm.PolygonLayer(polygons: _dangerFmPolygons),
                                                            if (userDetails['role'] == 'driver' &&
                                              ...
                                              (userDetails['role'] ==
                                                ...
                                                left: 0,
                                                right: 0,
                                                child: _bippDriverTopPanel(media),
                                              ),
                                              // Barra inferior futurista (solo home)
                                              driverReq.isEmpty ? _bippExpandableNavDock(media) : const SizedBox.shrink(),
                                              ...
                                              Positioned(
                                                  bottom: driverReq.isEmpty
                                                      ? (media.width * 0.20 + _safeBottomInset)
                                                      : (_safeBottomInset + (media.width * 0.02)),
                                                  child: Column(
                                                    crossAxisAlignment:
                                                    CrossAxisAlignment
                                                        .end,
                                                                        ...
                                                                        Row(
                                                                          children: [
                                                                            Container(
                                                                              ...
                                                                              decoration: BoxDecoration(shape: BoxShape.circle, image: DecorationImage(image: CachedNetworkImageProvider(choosenRide[0]['user_img'].toString()), fit: BoxFit.cover)),
                                                                            ),
                                                                  ...
                                                                  Container(
                                                                    ...
                                                                    child: Column(
                                                                      children: [
                                                                          ...
                                                                          child: Row(
                                                                            children: [
                                                                              Container(
                                                                                height: media.width * 0.2,
                                                                                width: media.width * 0.2,
                                                                                decoration: BoxDecoration(image: DecorationImage(image: CachedNetworkImageProvider(driverReq['userDetail']['data']['profile_picture'].toString()), fit: BoxFit.cover), borderRadius: BorderRadius.circular(media.width * 0.02), boxShadow: [BoxShadow(blurRadius: 2, color: Colors.white.withOpacity(0.2), spreadRadius: 2)], gradient: _bippGreenFadeGradient(),),
                                                                              ),
                                                                              SizedBox(
                                                                                width: media.width * 0.05,
                                                                              ),
                                                                                  ...
                                                                                  child: Column(
                                                                                    ...
                                                                                    children: [
                                                                                      ...
                                                                                      Wrap(
                                                                                        ...
                                                                                        runSpacing: media.width * 0.01,
                                                                                        children: [
                                                                                          MyText(
                                                                                            color: Colors.white,
                                                                                            text: (aproximateDistance != null) ? '${aproximateDistance.toStringAsFixed(2)} km' : '0.00 km',
                                                                                            size: media.width * fourteen * 1.15,
                                                                                          ),
                                                                                          MyText(
                                                                                          ...
                                                                                          Container(height: 10, width: 3, color: buttonColor),
                                                                                          MyText(
                                                                                            color: Colors.white,
                                                                                            text: aproximateMins,
                                                                                            size: media.width * fourteen,
                                                                                          ...
                                                                                          MyText(
                                                                                            color: Colors.white.withOpacity(0.9),
                                                                                            text: '${_pickupToDropKm().toStringAsFixed(1)} km  A DESTINO',
                                                                                            size: media.width * fourteen * 1.15,
                                                                                            fontweight: ui.FontWeight.w600,
                                                                                          ),
                                                                                        ],
                                                                        ...
                                                                        Builder(
                                                                          builder: (context) {
                                                                            final double totalSec = double.tryParse(userDetails['trip_accept_reject_duration_for_driver'].toString()) ?? 40.0;
                                                                            final double durNow = duration.toDouble();
                                                                            final double remaining = (totalSec - durNow).clamp(0.0, totalSec).toDouble();
                                                                            final double frac = (totalSec <= 0.0) ? 0.0 : (remaining / totalSec);
                                                                            final int remInt = remaining.round();
                                                                            return Row(
                                                                              children: [
                                                                                Expanded(
                                                                                  child: LayoutBuilder(
                                                                                ...
                                                                                Padding(
                                                                                  ...
                                                                                  child: Row(
                                                                                    children: [
                                                                                      ...
                                                                                      MyText(
                                                                                        color: (_isDangerousPickup()) ? const Color(0xFFFF3B30) : const Color(0xFF34C759),
                                                                                        text: (_isDangerousPickup()) ? 'Peligroso' : 'Seguro',
                                                                                        size: media.width * fourteen,
                                                                                        fontweight: ui.FontWeight.bold,
                                                                                      ),
                                                                                      MyText(
                                                                                      ...
                                                                                      MyText(
                                                                                        color: (_isDangerousDrop()) ? const Color(0xFFFF3B30) : const Color(0xFF34C759),
                                                                                        text: (_isDangerousDrop()) ? 'Peligroso' : 'Seguro',
                                                                                        size: media.width * fourteen,
                                                                                        fontweight: ui.FontWeight.bold,
                                                                                      ),
                                                                                    ],
                                                                                  ...
                                                                                  children: [
                                                                                    ...
                                                                                    Expanded(
                                                                                      child: MyText(
                                                                                        ...
                                                                                        size: media.width * (fourteen * 0.92),
                                                                                        // overflow: TextOverflow.ellipsis,
                                                                                        // maxLines: 1,
                                                                                      ),
                                                                                    ),
                                                                          ...
                                                                          children: [
                                                                            ...
                                                                            Button(
                                                                              onTap: () async {
                                                                                ...
                                                                                final String _acceptTryId = _bippCurrentRideKey();

                                                                                // Si el contador llegó a 0 o ya no hay request, casi seguro alguien más lo tomó
                                                                                if (duration == 0.0 || _acceptTryId.isEmpty) {
                                                                                  _showRideTakenMessage('Este viaje ya fue tomado por otro conductor.');
                                                                                  setState(() {
                                                                                    _isLoading = false;
                                                                                  });
                                                                                  return;
                                                                                ...
                                                                                // Confirmar aceptación: esperamos a que accepted_at se setee (sin bloquear mucho)
                                                                                bool _accepted = false;
                                                                                for (int _i = 0; _i < 5; _i++) {
                                                                                  await Future.delayed(const Duration(milliseconds: 500));
                                                                                  if (!mounted) break;

                                                                                  // Si cambió el pedido en pantalla, salimos
                                                                                  final String _curId = _bippCurrentRideKey();
                                                                                  if (_curId.isNotEmpty && _curId != _acceptTryId) {
                                                                                    _accepted = true; // ya no es el mismo, no mostramos error
                                                                                    break;
                                                                                  }
                                              ...
                                              (driverReq['accepted_at'] !=
                                                  ...
                                                  GestureDetector(
                                                    onVerticalDragStart:
                                                      ...
                                                      // Si está null, lo arrancamos abierto (más arriba)
                                                      addressBottom ??= _panelOpenHidden;
  ...
  Widget _bippFuturisticBottomBar(Size media) {
    final double _bippBottomInset = _systemBottomInset();
    final double _bippBaseHeight = media.width * 0.18;
    ...
    return Positioned(
      ...
      child: ClipRRect(
        ...
        child: BackdropFilter(
          ...
          child: Container(
            // La barra se “pega” al fondo, y el contenido se eleva con el inset real.
            height: _bippBaseHeight + _bippBottomInset + 6,
            padding: EdgeInsets.fromLTRB(10, 6, 10, _bippBottomInset + 6),
            decoration: BoxDecoration(
  ...
  // =============================================================
  // Dock flotante +/− (debajo del botón de centrar) - SOLO HOME
  // - Colapsado: muestra solo el botón "+"
  // - Expandido: muestra Inicio / Historia / Ganancias / Viajes
  // =============================================================
  Widget _bippExpandableNavDock(Size media) {
    final double inset = _systemBottomInset();

    // Alto/diámetro del botón (adaptativo)
    final double h = (media.width * 0.15).clamp(54.0, 64.0);
    final double toggleSize = h;

    // Más arriba (debajo del botón de centrar)
    final double _bippDockLift = (media.width * 0.125).clamp(48.0, 74.0);

    // Ancho expandido (cap)
    final double expandedW = (media.width * 0.78).clamp(260.0, 380.0);
    final double collapsedW = toggleSize;

    return Positioned(
      right: media.width * 0.04,
      bottom: inset + _bippDockLift,
      child: ClipRRect(
        ...
        child: BackdropFilter(
          ...
          child: AnimatedContainer(
            ...
            width: _navDockExpanded ? expandedW : collapsedW,
            height: h,
            decoration: BoxDecoration(
              color: Colors.black.withOpacity(0.26),
              borderRadius: BorderRadius.circular(h / 2),
            ...
            child: _navDockExpanded
                ? Row(
              children: [
                // Contenido (aparece hacia la izquierda del botón)
                Expanded(
                  child: ClipRect(
                ...
                // Botón + / -
                _bippDockToggleButton(toggleSize),
              ],
            )
                : Center(
              child: _bippDockToggleButton(toggleSize),
            ),
          ),
        ),
      ),
  ...
  Widget _bippDockToggleButton(double size) {
    return _bippCyberMapButton(
      size: size,
      icon: _navDockExpanded ? Icons.remove_rounded : Icons.add_rounded,
      iconSize: size * 0.56,
      onTap: () {
        setState(() {
          _navDockExpanded = !_navDockExpanded;
        });
      },
    );
  }

  Widget _bippDockNavItem({
    required IconData icon,
    required String label,
    required VoidCallback onTap,
    bool active = false,
  }) {
    final c = active ? const Color(0xff00FFB0) : Colors.white.withOpacity(0.88);

    return Expanded(
      child: InkWell(
        onTap: () {
          onTap();
          // al elegir una opción, contraer (queda más prolijo)
          setState(() {
            _navDockExpanded = false;
          });
        },
  ...
  Future<void> _bippOnConnectTap() async {
              ...
              Positioned(
                ...
                child: Material(
                  ...
                  child: InkWell(
                    onTap: () {
                      setState(() {
                        _bippTopPanelExpanded = !_bippTopPanelExpanded;
                      });
                    },
                    ...
                    child: Container(
                      ...
                      child: Icon(
                        _bippTopPanelExpanded ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded,
                        color: Colors.white,
...
class _DangerZonesParsed {
  final Set<Polygon> googlePolygons;
  final Set<Circle> googleCircles;
  final Set<Marker> googleMarkers;

  final List<fm.Polygon> fmPolygons;
  final List<fm.CircleMarker> fmCircles;
  final List<fm.Marker> fmMarkers;

  const _DangerZonesParsed({
    required this.googlePolygons,
    required this.googleCircles,
    required this.googleMarkers,
    required this.fmPolygons,
    required this.fmCircles,
    required this.fmMarkers,
  });
}

// ======================================================
// BIP Voz (IA) - Clases embebidas (NO crear archivos extra)
// ======================================================

class _BippTtsService {
  ...
  Future<void> init() async {
    try {
      ...
      await _tts.setPitch(1.05);      // delicada
      await _tts.setVolume(1.0);

      // Intentar elegir una voz es-AR (y si hay, femenina) en el dispositivo
      ...
      if (voices is List) {
        ...
        // Preferimos femenina si existe
        final female = esAr.where((v) {
          if (v is Map) {
            final g = (v['gender'] ?? v['sex'] ?? '').toString().toLowerCase();
            return g.contains('female') || g.contains('femen');
          }
          return false;
        }).toList();

        if (female.isNotEmpty && female.first is Map) {
          chosen = Map<String, dynamic>.from(female.first as Map);
        } else if (esAr.isNotEmpty && esAr.first is Map) {
          chosen = Map<String, dynamic>.from(esAr.first as Map);
        }

        if (chosen != null) {
          try {
            final voiceMap = <String, String>{};
            if (chosen['name'] != null) voiceMap['name'] = chosen['name'].toString();
            if (chosen['locale'] != null) voiceMap['locale'] = chosen['locale'].toString();
            if (chosen['language'] != null && !voiceMap.containsKey('locale')) {
              voiceMap['locale'] = chosen['language'].toString();
            }
  ...
  Future<void> speak(
    ...
    // Cooldown por key
    if (!force && _lastKey == key && _lastAt != null) {
      final diff = now.difference(_lastAt!).inMilliseconds;
      if (diff < cooldownMs) return;
    }

    // Si ya está hablando, no spamear (salvo force)
...
class _BippTripVoiceAnnouncer {
  final _BippTtsService tts;
  _BippTripVoiceAnnouncer(this.tts);
  ...
  int _lastEvalMs = 0;
  bool _saidTaken = false;


  void resetForTrip(String tripKey) {
    _cancelled = false;
    _saidTaken = false;
    _tripKey = tripKey;
    _saidNew = false;
  ...
  Future<void> onNewTrip(String tripKey) async {
    if (_tripKey != tripKey) resetForTrip(tripKey);
    if (_saidNew) return;
    _saidNew = true;
    await tts.speak("Neeeeñooo Tenes un nuevo viaje.", key: "new_$tripKey");
  }

  Future<void> onCancelled() async {
    if (_tripKey == null) return;
    if (_saidCancel) return;
    _saidCancel = true;
    _cancelled = true;

    // Cancelación tiene prioridad: corta lo que esté hablando y avisa.
    await tts.speak(
      "El pasajero canceló el viaje.",
      key: "cancel_${_tripKey!}",
      cooldownMs: 800,
      force: true,
    );
  }
  ...
  Future<void> onTakenByOther() async {
    // Puede pasar sin tripKey (ej: se fue el request). Igual avisamos 1 vez.
    if (_saidTaken) return;
    _saidTaken = true;

    await tts.speak(
      "Este viaje ya fue tomado por otro conductor.",
      key: "taken_${_tripKey ?? 'x'}",
      cooldownMs: 2500,
      force: true,
    );
  }
  Future<void> onLocationUpdate({
    ...
    final nowMs = DateTime.now().millisecondsSinceEpoch;
    if (nowMs - _lastEvalMs < 850) return;
    _lastEvalMs = nowMs;
    if (accepted && !tripStarted) {
      final dPick = geolocator.Geolocator.distanceBetween(myLat, myLng, pickLat, pickLng);
      if (dPick <= 150 && !_said150Pickup) {
        _said150Pickup = true;
        await tts.speak("Estás por llegar a buscar al pasajero.", key: "p150_${_tripKey!}");
      }
      return;
    }

    if (tripStarted) {
      final dDrop = geolocator.Geolocator.distanceBetween(myLat, myLng, dropLat, dropLng);

      if (dDrop <= 150 && !_said150Drop) {
        _said150Drop = true;
        await tts.speak("Estás por finalizar el viaje.", key: "d150_${_tripKey!}");
      }

      if (dDrop <= 50 && !_said50Drop) {
        _said50Drop = true;
        await tts.speak("Señores Pasajero, Revise no dejar sus pertenencias", key: "d50_${_tripKey!}");
      }
    }
  }
}
