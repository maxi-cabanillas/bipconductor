...
//pick image from camera
  ...
  // =============================================================
  // Registro: foto obligatoria (cámara)
  // =============================================================
  bool _ensureProfilePhotoRequired() {
    // Solo aplica al registro (signIn == 1)
    if (signIn != 1) return true;

    final hasPhoto = (proImageFile1 != null &&
        proImageFile1.toString().trim().isNotEmpty);

    if (hasPhoto) return true;

    // Mensaje visible + abrir cámara (obligatorio)
    try {
      setState(() {
        loginLoading = false;
        _error = '📸 Es obligatorio sacarte una foto para registrarte.';
        pickImage = true; // muestra el panel de selección
      });
    } catch (_) {}

    // Intentar abrir cámara automáticamente (si el usuario niega permiso, queda el panel abierto)
                                        ...
                                        onTap: () async {
                                          ...
                                          if (newPassword == true) {
                                            ...
                                            if (mobileVerified == true) {
                                              ...
                                              if (signIn == 1 && !_ensureProfilePhotoRequired()) {
                                                return;
                                              }
